...
object Day09 {
  ...
  private def doFirstInvalidNumber(window: Int, values: List[Long]): Long =
    (0 until values.size - window - 1)
      ...
      .filter { case (expectedNumber, subList) => isInvalid(expectedNumber, subList) }
      .map(_._1)
      .head
  ...
  def doFindContiguousSet(currentIndex: Int, expectedSum: Long, values: List[Long]): Option[List[Long]] =
    (2 to values.size)
