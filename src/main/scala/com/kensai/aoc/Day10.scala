...
object Day10 {
  ...
  @tailrec
  def doComputeDiff(currentValue: Long, acc: (Long, Long, Long), values: Set[Long]): (Long, Long, Long) = {
   ...
   */
  @tailrec
  private def doCompute2(currentKey: Long, max: Long, acc: Map[Long, Long], values: Set[Long]): Long = {
    if (currentKey >= max)
      acc(max)
    else {
      if (acc.contains(currentKey)) {
        val newState = (2 to 3).foldLeft(update(currentKey, acc, 1))(update(currentKey, _, _))
  ...
  private def update(key: Long, acc: Map[Long, Long], keyInc: Int): Map[Long, Long] =
