...
import com.kensai.aoc.Day14.Mask.valueToBitSet
...
object Day14 {
  ...
  case class Mask(maskStr: String, positiveMask: BitSet, negativeMask: BitSet) {
     ...
     */
    private def generateFloatingBitSets(acc: List[BitSet], floatingIndex: Int): List[BitSet] =
  ...
  object Mask {
    def valueToBitSet(input: String, keepPositive: Boolean): BitSet =
      input
        .reverse
        .zipWithIndex.filter(i => if (keepPositive) i._1 == '1' else i._1 == '0')
  ...
  def parse(inputs: String): List[Input] =
    inputs
      ...
      .map(other => {
        ...
        val instructions = rows.tail.map {
          case memRegex(memoryAddress, valueToApply) => Instruction(memoryAddress.toLong, valueToApply.toLong)
        }.toList
  ...
  def doComputePart1(input: Input): Map[Long, Long] =
    input
      ...
      .map { case (memoryAddress, instructions) => (memoryAddress, instructions.last) }
      .map { case (memoryAddress, instr) => (memoryAddress, input.mask.computeLong(instr.valueToApply)) }

  def doComputePart2(input: Input): Map[Long, Long] =
    input
      .instructions
      .flatMap(instr => input.mask.computeAddresses(instr.memoryAddress).map((_, instr.valueToApply)))
      .groupBy(_._1)
      .map { case (memoryAddress, values) => memoryAddress -> values.map(_._2).last }
