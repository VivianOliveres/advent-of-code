...
object Day16 {
  ...
  case class Board(rules: List[Rule], yourTicket: Ticket, nearbyTickets: List[Ticket])
  ...
  case class Rule(name: String, min1: Long, max1: Long, min2: Long, max2: Long) {
  ...
  def doParseRule(line: String): Rule = line match {
    case ruleRegex(name, min1, max1, min2, max2) => Rule(name, min1.toLong, max1.toLong, min2.toLong, max2.toLong)
  }

  def parse(inputs: String): Board = {
    val splited = inputs.split("\n\n").toList
    val rules = splited.head.split("\n").map(_.trim).filterNot(_.isEmpty).map(doParseRule).toList
    val yourTicketPart = splited(1).split("\n").map(_.trim).filterNot(_.isEmpty).tail.map(Ticket(_)).head
    val nearbyTicketPart = splited(2).split("\n").map(_.trim).filterNot(_.isEmpty).tail.map(Ticket(_)).toList
    Board(rules, yourTicketPart, nearbyTicketPart)
  }

  def doSumFirstInvalidValues(ticket: Ticket, rules: List[Rule]): Long = {
    ticket.values.map(value => rules.find(_.isValid(value)).map(_ => 0L).getOrElse(value)).sum
  }
  ...
  def doFieldsPosition(ticket: Ticket, rules: List[Rule]): Option[Map[Int, Set[Rule]]] = {
    val result = ticket
      .values
      .zipWithIndex
  ...
  def replaceSolutions(input: Map[Int, Set[Rule]]): Map[Int, Set[Rule]] = {
    ...
    var toRemove = input.filter(_._2.size == 1).flatMap(_._2).filterNot(rule => alreadyReduced.contains(rule)).toList
    while(toRemove.nonEmpty) {
      ...
      toRemove = input.filter(_._2.size == 1).flatMap(_._2).filterNot(rule => alreadyReduced.contains(rule)).toList
    }
    tmp
  }

  def reduce(i1: Map[Int, Set[Rule]], i2: Map[Int, Set[Rule]]): Map[Int, Set[Rule]] = {
  ...
  def doFieldsPosition(tickets: List[Ticket], rules: List[Rule]): Map[Int, Set[Rule]] = {
  ...
  def fieldsPosition(input: String): Long = {
    ...
    doFieldsPosition(board.nearbyTickets, board.rules)
      ...
      .map{case (index, _) => board.yourTicket.values(index)}.foldLeft(1L)(_ * _)
  }
}
