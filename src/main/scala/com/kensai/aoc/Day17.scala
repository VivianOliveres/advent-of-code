...
object Day17 {
  ...
  def isCubeActivated[T](coord: T, activeCoord: Set[T], neighbors: Int): Boolean =
    if (activeCoord.contains(coord))
      neighbors == 2 || neighbors == 3
    else
      neighbors == 3

  def doComputeNextState[T](activeCoord: Set[T], neighborsGenerator: T => Seq[T]): Set[T] =
    activeCoord
      .toList
      .flatMap(neighborsGenerator)
      .groupBy(x => x)
      .map{case (coord, all) => (coord, all.size)}
      .filter{case (coord, size) => isCubeActivated(coord, activeCoord, size)}
      .keySet
  ...
  def activeCubeAfter[T](neighborsGenerator: T => Seq[T])(numCycles: Int, cubes: Set[T]): Long =
    (0 until numCycles)
      .foldLeft(cubes){case (acc, _) => doComputeNextState(acc, neighborsGenerator)}
      .size
}
