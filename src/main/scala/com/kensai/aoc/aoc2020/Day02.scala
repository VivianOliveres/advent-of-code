...
object Day02 {
  ...
  def parse(input: String): Option[PasswordRow] =
    input match {
      case rowRegex(lowestValue, higherValue, value, password) =>
        Some(
          PasswordRow(
            lowestValue.toInt,
            higherValue.toInt,
  ...
  /** Check that input is a valid {@code PasswordRow}.<br> It checks that {@code PasswordRow.expectedChar} is present between {@code
    * PasswordRow.firstIndex} and {@code PasswordRow.lastIndex} times into {@code PasswordRow.password}.
    */
  ...
  /** Check that input is a valid {@code PasswordRow}.<br> It checks that {@code PasswordRow.expectedChar} is present at {@code
    * PasswordRow.firstIndex} or {@code PasswordRow.lastIndex} into {@code PasswordRow.password} but not at both positions.
    */
  def isPasswordValidPart2(candidate: PasswordRow): Boolean =
    candidate.password(
      candidate.firstIndex - 1
    ) == candidate.expectedChar ^ candidate.password(
      candidate.lastIndex - 1
    ) == candidate.expectedChar

  /** Count the number of valid {@code PasswordRow} from {@code candidates}.
    */
  def validPasswordCount(
      candidates: List[PasswordRow]
    )(validator: PasswordRow => Boolean
    ): Int =
