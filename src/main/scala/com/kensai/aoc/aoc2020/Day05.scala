...
object Day05 {
  ...
  /** Compute the row index from the given boarding pass.
    */
  def computeRow(boardingPass: String): Long =
    doCompute(boardingPass.substring(0, boardingPass.length - 3), 0, 127)
  ...
  private def doComputeUpper(
    ...
    doCompute(
      boardingPass.tail,
      firstIndex,
      ((firstIndex + lastIndex) / 2.0).round
    )

  private def doComputeLower(
      boardingPass: String,
      firstIndex: Long,
      lastIndex: Long
    ): Long =
