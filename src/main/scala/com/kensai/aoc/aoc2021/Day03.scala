...
object Day03 {
  ...
  def computePowerConsumption(inputs: Seq[String]): Int = {
    ...
    val (gamma, epsilon) = countByIndex
      .map { case (index, (ones, zeros)) =>
        val bit         = ones >= zeros
  ...
  private def bitsToLong(bits: Seq[Boolean]): Long = bits.zipWithIndex.foldLeft(0L) { case (acc, (bit, index)) =>
    acc + bitToInt(bit) * math.pow(2.0, bits.size - index - 1.0).toInt
  }
