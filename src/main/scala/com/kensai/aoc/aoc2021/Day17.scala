...
object Day17 {
  ...
  /**
    * Generate all the initial velocities that will goes threw the target square. But return only the one that goes to the highest Y coordinates
    * @return ((xInitVelocity, yInitVelocity), maxYReached)
    */
  ...
  /**
    ...
    */
  @tailrec
  private def findValidVelocity(initVelocity: (Int, Int), currentVelocity: (Int, Int), currentPos: (Int, Int), maxYReached: Int, inputs: Inputs): Option[((Int, Int), Int)] = {
    if (currentPos._2 < inputs.minY ) // Too far away on the y axis
      ...
      val (newX, newXVelocity) = if (currentVelocity._1 == 0) (currentPos._1, 0) else (currentPos._1 + currentVelocity._1, currentVelocity._1 - 1)
