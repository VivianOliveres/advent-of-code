...
object Day22 {
  ...
  def parse(rows: Seq[String]): Day22Input = {
    val instructions = rows.map(_.trim).map {
      case onRegex(onOff, minX, maxX, minY, maxY, minZ, maxZ) =>
        Instruction(onOff == "on", Cube3D(minX.toInt, maxX.toInt, minY.toInt, maxY.toInt, minZ.toInt, maxZ.toInt))
      ...
      cubes.zipWithIndex.foreach { case (maybeCube, index) =>
        ...
        if (!filtering(instr, cube)) {
          ...
          if (instr.cube3D.min.y > cube.min.y)
            cubes = cubes :+ Some(
              Cube3D(
                max(cube.min.x, instr.cube3D.min.x),
                min(cube.max.x, instr.cube3D.max.x),
                cube.min.y,
                instr.cube3D.min.y - 1,
                cube.min.z,
                cube.max.z
              )
            )
          ...
          if (instr.cube3D.min.z > cube.min.z)
            cubes = cubes :+ Some(
              Cube3D(
                max(cube.min.x, instr.cube3D.min.x),
                min(cube.max.x, instr.cube3D.max.x),
                max(cube.min.y, instr.cube3D.min.y),
                min(cube.max.y, instr.cube3D.max.y),
                cube.min.z,
                instr.cube3D.min.z - 1
              )
            )
