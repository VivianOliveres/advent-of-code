...
object Day15 {
  ...
  private def computeRanges(sensors: Seq[Sensor], yTarget: Int): Seq[RangeDay15] =
    (for {
      sensor <- sensors
  ...
  def countInvalidPositions(sensors: Seq[Sensor], yTarget: Int): Int = {
  ...
  def findValidPosition(sensors: Seq[Sensor], yTarget: Int, maxValue: Int): Option[Point2D] = {
    val ranges       = computeRanges(sensors, yTarget)
  ...
  def tuningFrequency(sensors: Seq[Sensor], maxValue: Int): Long = {
  ...
  @tailrec
  private def searchValidPositions(sensors: Seq[Sensor], maxValue: Int, remaining: Seq[Int]): Point2D = {
