...
class Day01Suite extends AnyFlatSpec {
  ...
  "compute3" should "find result from first example" in {
    // GIVEN: input from specs
    val input = List(1721L, 979L, 366L, 299L, 675L, 1456L)
    ...
    // THEN: result is 241861950
    val expectedResult = Some(241861950)
    result should be (expectedResult)
  }

  "compute3" should "return empty if there is no result" in {
    // GIVEN: input from specs
    val input = List(3L, 5L, 18L)
  ...
  "compute3" should "return empty for an empty list" in {
    // GIVEN: input from specs
    val input: List[Long] = List()
