...
class Day11PropSpec extends AnyPropSpec with TableDrivenPropertyChecks with Matchers with GivenWhenThen with Day11Fixtures {
  ...
  property("parse should return valid Board") {
    val inputs = Table(
      ...
      SimpleTestParam("L.#", Board(1, 3, Map(Pos(0, 0) -> 'L', Pos(0, 2) -> '#'))),
  ...
  property("extractAdjacent should return valid List") {
    ...
    case class ExtractAdjacentTestParam(input: String, pos: Pos, expectedResult: List[Char])
    ...
    val inputs = Table(
      ...
      ExtractAdjacentTestParam("LL#\nL.#\nL##\n", Pos(1, 1), List('L', 'L', 'L', 'L', '#', '#', '#', '#'))
    )
  ...
  property("computeSwitch should return new char for count=4 and extractAdjacent") {

    case class ComputeSwitchTestParam(input: String, pos: Pos, expectedResult: Option[(Pos, Char)])

    val inputs = Table(
      "Test parameters",
      ComputeSwitchTestParam("###\n#L#\n###", Pos(1, 1), None),
      ComputeSwitchTestParam("###\n###\n###", Pos(1, 1), Some((Pos(1, 1), 'L'))),
      ComputeSwitchTestParam("###\n.#.\nLLL", Pos(1, 1), None)
    )
  ...
  property("doComputeNextBoard should return valid adjacent") {
    ...
    case class ComputeNextBoardTestParam(input: String, expectedCount: Int, expectedResult: String)

    val inputs = Table(
      "Test parameters",
      ComputeNextBoardTestParam("L.#.L\n.....\n#.L.#\n.....\nL.#.L\n", 5, "L.#.L\n.....\n#.L.#\n.....\nL.#.L\n"), // Same board
    ...
    forAll(inputs) { param =>
      ...
      val result = doComputeNextBoard(board, param.expectedCount, extractVisibleAdjacent)
