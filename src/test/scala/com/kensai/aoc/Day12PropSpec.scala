...
class Day12PropSpec extends AnyPropSpec with TableDrivenPropertyChecks with Matchers with GivenWhenThen {
  ...
  property("move should return valid Position") {
    ...
    case class MoveTestParam(initialPosition: Pos, command: Command, expectedPosition: Pos)
    ...
    forAll(inputs) { param =>
      Given(s"Initial position is ${param.initialPosition} and command is ${param.command}")
  ...
  property("doMove2 should return valid Position and Waypoint") {
    ...
    case class Move2TestParam(initialPosition: Pos, initialWaypoint: Pos, command: Command, expectedPosition: Pos, expectedWaypoint: Pos)

    val inputs = Table(
      "Test parameters",
      Move2TestParam(Pos(0, 0, E), Pos(10, 1, N), Forward(10), Pos(100, 10, E), Pos(10, 1, N)),
      Move2TestParam(Pos(100, 10, E), Pos(10, 1, N), North(3), Pos(100, 10, E), Pos(10, 4, N)),
      Move2TestParam(Pos(100, 10, E), Pos(10, 4, N), Forward(7), Pos(170, 38, E), Pos(10, 4, N)),
      Move2TestParam(Pos(170, 38, E), Pos(10, 4, N), Right(90), Pos(170, 38, E), Pos(4, -10, N))
    )

    forAll(inputs) { param =>
      Given(s"Pos[${param.initialPosition}] Waypoint[${param.initialWaypoint}] Command[${param.command}]")

      When(s"move(${param.initialPosition}, param.initialWaypoint, ${param.command})")
      val result = doMove2(param.initialPosition, param.initialWaypoint, param.command)

      Then(s"result should be Pos[${param.expectedPosition}] Waypoint[${param.expectedWaypoint}]")
