...
class Day16Spec extends AnyFlatSpec with GivenWhenThen {
  ...
  private lazy val puzzleInputs = readInputFile("src/test/resources/Day16.input")
  private lazy val specInputs = readInputFile("src/test/resources/Day16Spec.input")
  private lazy val spec2Inputs = readInputFile("src/test/resources/Day16Spec2.input")
  ...
  "parse for spec" should "return solution" in {
    ...
    result shouldBe Board(
      List(Rule("class", 1, 3, 5, 7), Rule("row", 6, 11, 33, 44), Rule("seat", 13, 40, 45, 50)),
      Ticket(List(7L,1L, 14L)),
      List(Ticket(List(7L, 3L, 47L)), Ticket(List(40L, 4L, 50L)), Ticket(List(55L, 2L, 20L)), Ticket(List(38L, 6L, 12L)))
    )
  }

  "doSumFirstInvalidValues for valid ticket" should "return 0" in {
    Given(s"Input is spec")
    val rules = List(Rule("class", 1, 3, 5, 7), Rule("row", 6, 11, 33, 44), Rule("seat", 13, 40, 45, 50))
  ...
  "doFieldsPosition for valid ticket" should "return Some(result)" in {
    ...
    Then(s"Result is None")
    result shouldBe Some(Map(0 -> Set(ruleClass), 1 -> Set(ruleRow), 2 -> Set(ruleSeat)))
  }
  ...
  "doFieldsPosition XYZXYZ" should "return Some(result)" in {
    ...
    result shouldBe Some(Map(0 -> Set(rule0, rule2), 1 -> Set(rule0, rule1, rule2), 2 -> Set(rule0, rule1, rule2)))
  }
  ...
  "doFieldsPosition XYZ" should "return Some(result)" in {
    ...
    Then(s"Result is ")
    result shouldBe Map(0 -> Set(rule0, rule2), 1 -> Set(rule0, rule1, rule2), 2 -> Set(rule0, rule1, rule2))
  }
  ...
  "doFieldsPosition for ticket 2" should "return Some(result)" in {
    ...
    Then(s"Result is ")
    result shouldBe Map(0 -> Set(rule0, rule1), 1 -> Set(rule0, rule1, rule2), 2 -> Set(rule0, rule1, rule2))
  }
  ...
  "doFieldsPosition for 2 tickets" should "return Some(result)" in {
    ...
    Then(s"Result is ")
    result shouldBe Map(0 -> Set(rule0), 1 -> Set(rule1, rule2), 2 -> Set(rule1, rule2))
  }
  ...
  "reduce for spec" should "return Some(result)" in {
    ...
    val ticket1 = Ticket(List(3L,9L,18L))
    val r1 = List(ticket1).flatMap(doFieldsPosition(_, rules)).foldLeft(first)(reduce)
    r1 shouldBe Map(0 -> Set(rule0, rule2), 1 -> Set(rule0, rule1, rule2), 2 -> Set(rule0, rule1, rule2))

    val ticket2 = Ticket(List(15L,1L,5L))
    val r2 = List(ticket2).flatMap(doFieldsPosition(_, rules)).foldLeft(r1)(reduce)
    r2 shouldBe Map(0 -> Set(rule0), 1 -> Set(rule1, rule2), 2 -> Set(rule1, rule2))

    val ticket3 = Ticket(List(5L,14L,9L))
    val r3 = List(ticket3).flatMap(doFieldsPosition(_, rules)).foldLeft(r2)(reduce)
